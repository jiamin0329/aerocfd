...
!! DATE:   2012.11.28
!!=========================================================
subroutine UpdateBufferJacobian1
	use blk_var
	use index_var
	use mpi_var
	use glbindex_var
				...
				if     (face .eq. 1)then	!!face i-
					...
					do j = js_bc,je_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,:,is_bc,j,k) = blk(m0)%dxidx(:,is_bc+nBuffer,j,k)
						end do 
					end do
					end do
					...
					do j = js_bc,je_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,:,ie_bc,j,k) = blk(m0)%dxidx(:,ie_bc-nBuffer,j,k)
						end do
					end do
					end do					
					...
					do i = is_bc,ie_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,:,i,js_bc,k) = blk(m0)%dxidx(:,i,js_bc+nBuffer,k)
						end do 
					end do
					end do				
					...
					do i = is_bc,ie_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,:,i,je_bc,k) = blk(m0)%dxidx(:,i,je_bc-nBuffer,k)
						end do
					end do
					end do
					...
					do i = is_bc,ie_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,:,i,j,ks_bc) = blk(m0)%dxidx(:,i,j,ks_bc+nBuffer)
						end do 
					end do		
					end do
					...
					do i = is_bc,ie_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,:,i,j,ke_bc) = blk(m0)%dxidx(:,i,j,ke_bc-nBuffer)
						end do
					end do
					end do
				end if
			...
			if (blk_t .ge. 0) then
				...
				if (face .eq. 1) then !!boundary  i- 
					...
					do j=1,l2
						...
						do nBuffer = 1, bufferLength
							blk(m0)%dxidx(:,i0-nBuffer,j0,k0) = ua2(nBuffer,:,j,k)
						end do 
					end do
					end do
				end if
				...
				if (face .eq. 3) then	!!boundary  j-
					...
					do i=1,l2
						...
						do nBuffer = 1, bufferLength
							blk(m0)%dxidx(:,i0,j0-nBuffer,k0) = ua2(nBuffer,:,i,k)
						end do 
					end do
				end do
				end if
