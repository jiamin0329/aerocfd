...
!! DATE:   2012.11.28
!!=========================================================
subroutine UpdateBufferCoordinate
	use blk_var
	use index_var
	use mpi_var
	use glbindex_var
		...
		do ksub = 1,blk(m0)%num_subface
			...
			if (blk_t .ge. 0) then
				...
				if     (face .eq. 1)then	!!face i-
					...
					do i = is_bc,ie_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,1,i,j,ks_bc) = blk(m0)%x(i,j,ks_bc+nBuffer)
							blk(m0)%bc(ksub)%buffer_send(nBuffer,2,i,j,ks_bc) = blk(m0)%y(i,j,ks_bc+nBuffer)
							blk(m0)%bc(ksub)%buffer_send(nBuffer,3,i,j,ks_bc) = blk(m0)%z(i,j,ks_bc+nBuffer)
						end do 
					end do		
					end do			
					...
					do i = is_bc,ie_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,1,i,j,ke_bc) = blk(m0)%x(i,j,ke_bc-nBuffer)
							blk(m0)%bc(ksub)%buffer_send(nBuffer,2,i,j,ke_bc) = blk(m0)%y(i,j,ke_bc-nBuffer)
							blk(m0)%bc(ksub)%buffer_send(nBuffer,3,i,j,ke_bc) = blk(m0)%z(i,j,ke_bc-nBuffer)
						end do
					end do
					end do
				end if
			...
			if (blk_t .ge. 0) then
				...
				if (face .eq. 1) then !!boundary  i- 
					...
					do j=1,l2
						...
						do nBuffer = 1, bufferLength
							blk(m0)%x(i0-nBuffer,j0,k0) = ua2(nBuffer,1,j,k)
							blk(m0)%y(i0-nBuffer,j0,k0) = ua2(nBuffer,2,j,k)
							blk(m0)%z(i0-nBuffer,j0,k0) = ua2(nBuffer,3,j,k)
						end do 
					end do
					end do
				end if
				...
				if (face .eq. 3) then	!!boundary  j-
					...
					do i=1,l2
						...
						do nBuffer = 1, bufferLength
							blk(m0)%x(i0,j0-nBuffer,k0) = ua2(nBuffer,1,i,k)
							blk(m0)%y(i0,j0-nBuffer,k0) = ua2(nBuffer,2,i,k)
							blk(m0)%z(i0,j0-nBuffer,k0) = ua2(nBuffer,3,i,k)
						end do 
					end do
					end do
				end if
