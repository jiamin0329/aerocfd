	...
	do m0 = 1,blk_loop
		do ksub = 1,blk(m0)%num_subface
			...
			!!	             dt,timestep,d_0,u_0,p_0,gamma,ma,cv,face,is_bc,ie_bc,js_bc,je_bc,ks_bc,ke_bc,is,ie,js,je,ks,ke)
				...
				call inflow (blk(m0)%q,d_0,u_0,p_0,aoa,gamma,face,is_bc,ie_bc,js_bc,je_bc,ks_bc,ke_bc,is,ie,js,je,ks,ke)
			else if (blk(m0)%bc(ksub)%blk_t .eq. bc_outflow )then
				call outflow(blk(m0)%q,face,is_bc,ie_bc,js_bc,je_bc,ks_bc,ke_bc,is,ie,js,je,ks,ke)
			else if (blk(m0)%bc(ksub)%blk_t .eq. bc_symm    )then
				call symm(blk(m0)%q,blk(m0)%dxidx,face,is_bc,ie_bc,js_bc,je_bc,ks_bc,ke_bc,is,ie,js,je,ks,ke)
			else if (blk(m0)%bc(ksub)%blk_t .eq. bc_wall    )then
				if      (iflag_solver .eq. iflag_nssolver   )then
					call nonslipwall (blk(m0)%q,face,is_bc,ie_bc,js_bc,je_bc,ks_bc,ke_bc,is,ie,js,je,ks,ke)
				else if (iflag_solver .eq. iflag_eulersolver)then
					call slipwall(blk(m0)%q,blk(m0)%dxidx,face,is_bc,ie_bc,js_bc,je_bc,ks_bc,ke_bc,is,ie,js,je,ks,ke)
				end if
			else if (blk(m0)%bc(ksub)%blk_t .eq. bc_farfield)then
				call farfield (blk(m0)%q,blk(m0)%dxidx, &
