			...
			if (blk_t .ge. 0) then
				...
				if (face .eq. 1) then !!boundary  i- 
					...
					do j=1,nn
						j0 = js_bc+j-1
						do nBuffer = 1, bufferLength
							blk(m0)%q(1,i0-nBuffer,j0,k0) = ua2(nBuffer,1,j)
							blk(m0)%q(2,i0-nBuffer,j0,k0) = ua2(nBuffer,2,j)
				...
				if (face .eq. 3) then	!!boundary  j-
					...
					do i=1,nn
						i0 = is_bc+i-1
						do nBuffer = 1, bufferLength
							blk(m0)%q(1,i0,j0-nBuffer,k0) = ua2(nBuffer,1,i)
							blk(m0)%q(2,i0,j0-nBuffer,k0) = ua2(nBuffer,2,i)
