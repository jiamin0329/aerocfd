...
!! DATE:   2012.11.28
!!=========================================================
subroutine UpdateBufferJacobian2d2
	use blk_var
	use index_var
	use mpi_var
	use glbindex_var
	...
	do m0 = 1,blk_loop
		...
		do ksub=1,blk(m0)%num_subface
			...
			if (blk_t .ge. 0) then
				...
				allocate(ua2(bufferLength,1,nn))
				if      (ori .eq. 2)then
					do ll0 = 1,nn
						ua2(:,:,ll0) = ua(:,:,ll0)
					end do
				...
				if (face .eq. 1) then !!boundary  i- 
					...
					do j=1,nn
						j0 = js_bc+j-1
						do nBuffer = 1, bufferLength
							blk(m0)%inv_j(i0-nBuffer,j0,k0) = ua2(nBuffer,1,j)
						end do 
					end do
				end if
				!!*
				...
				if (face .eq. 3) then	!!boundary  j-
					...
					do i=1,nn
						i0 = is_bc+i-1
						do nBuffer = 1, bufferLength
							blk(m0)%inv_j(i0,j0-nBuffer,k0) = ua2(nBuffer,1,i)
						end do 
					end do
				end if
				!!*
