...
!! DATE:   2012.11.28
!!=========================================================
subroutine UpdateBufferJacobian2
	use blk_var
	use index_var
	use mpi_var
	use glbindex_var
				...
				if     (face .eq. 1)then	!!face i-
					...
					do j = js_bc,je_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,1,is_bc,j,k) = blk(m0)%inv_j(is_bc+nBuffer,j,k)
						end do 
					end do
					end do
					...
					do j = js_bc,je_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,1,ie_bc,j,k) = blk(m0)%inv_j(ie_bc-nBuffer,j,k)
						end do
					end do
					end do					
					...
					do i = is_bc,ie_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,1,i,js_bc,k) = blk(m0)%inv_j(i,js_bc+nBuffer,k)
						end do 
					end do
					end do				
					...
					do i = is_bc,ie_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,1,i,je_bc,k) = blk(m0)%inv_j(i,je_bc-nBuffer,k)
						end do
					end do
					end do
					...
					do i = is_bc,ie_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,1,i,j,ks_bc) = blk(m0)%inv_j(i,j,ks_bc+nBuffer)
						end do 
					end do		
					end do
					...
					do i = is_bc,ie_bc
						do nBuffer = 1, bufferLength
							blk(m0)%bc(ksub)%buffer_send(nBuffer,1,i,j,ke_bc) = blk(m0)%inv_j(i,j,ke_bc-nBuffer)
						end do
					end do
					end do
				end if
			...
			if (blk_t .ge. 0) then
				...
				if (face .eq. 1) then !!boundary  i- 
					...
					do j=1,l2
						...
						do nBuffer = 1, bufferLength
							blk(m0)%inv_j(i0-nBuffer,j0,k0) = ua2(nBuffer,1,j,k)
						end do 
					end do
					end do
				end if
				...
				if (face .eq. 3) then	!!boundary  j-
					...
					do i=1,l2
						...
						do nBuffer = 1, bufferLength
							blk(m0)%inv_j(i0,j0-nBuffer,k0) = ua2(nBuffer,1,i,k)
						end do 
					end do
				end do
				end if
				...
				if (face .eq. 5) then	!!boundary  k-
					...
					do i=1,l2
						...
						do nBuffer = 1, bufferLength
							blk(m0)%inv_j(i0,j0,k0-nBuffer) = ua2(nBuffer,1,i,k)
						end do
					end do
					end do
				end if
